...
function reName(name: string) {
  if (!name)
    name = 'jun-89'
  return name === 'jun-89' ? 'jun-89' : name
}
...
export async function genFeed(config: SiteConfig) {
  const feed = new Feed({
    ...
    language: 'zh-CN',
    image: '/satellite.svg',
    favicon: `${baseUrl}/satellite.svg`,
    copyright:
      'Copyright (c) 2023-present, 豫ICP备2023021681号',
  })
  ...
  for (const { url, frontmatter, html } of posts) {
    ...
    feed.addItem({
      ...
      author: [
        {
          name: frontmatter?.author || 'Jun',
          link: frontmatter?.author
            ? getGithubLink(frontmatter?.author)
            : undefined,
        },
      ],
      date: frontmatter?.date || new Date('2023-09-18'),
    })
  }
